...
fn large_file() {
    ...
    test_decrypt(&vec![file.as_path()], &["-k", "env:FED_E2E_LARGE_FILE_TESTKEY", "-d", "-v"], None, true);
    assert!(!enc_pth.as_path().exists());
    assert!(file.as_path().exists());
...
fn many_files() {
    ...
    test_decrypt(&paths, &["-k", "pipe", "-q", "-f"], Some(key.to_owned()), true);
...
fn output_dir_multi_salt() {
    ...
    for i in 0..5 {
        ...
        out_file.push(format!("{0}.{1}", in_file.file_name().unwrap().to_string_lossy(), &extension));
        test_encrypt(
            &vec![in_file.as_path()],
            &["-k", "pipe", "-v",
