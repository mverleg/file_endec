...
fn mixed_options() {
    ...
    for options in &option_sets {
        ...
        args.push("-k");
        args.push(key);
        test_encrypt(&[file.as_path()], &args, None);
        assert!(enc_pth.as_path().exists());
    ...
    let decrypt_paths = encrypted
        .iter()
        .map(|enc| enc.path.as_path())
        .collect::<Vec<_>>();
    test_decrypt(
        &decrypt_paths,
    ...
    encrypted.drain(..).for_each(|encrypt_info: Encrypted| {
        ...
        assert_eq!(
            fs::read(encrypt_info.path.as_path()).unwrap(),
            encrypt_info.data
        );
...
fn hiding() {
    ...
    for rep in 0..rep_count {
        ...
        assert!(!file.as_path().exists());
        let data_header = "\nmeta1+data:\n";
        let header_len = BufReader::new(File::open(enc_pth.as_path()).unwrap())
            .lines()
